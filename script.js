...
for (let i = 0; i < galaxyParameters.count; i++) {
  ...
  const randomY = (Math.random() - 1.5) * galaxyParameters.randomness * radius * 1.2; // thay từ 0.5 lên 1.5
...
function createTextRings() {
  ...
  for (let i = 0; i < numRings; i++) {
    ...
    ringGroup.userData = {
      ...
      speed: 0.0005 + 0.00025, // Tốc độ quay
  ...
  if (window.textRings) {
    ...
    window.textRings.forEach((ringGroup, index) => {
      ...
      if (textMesh && textMesh.material) {
        ...
        textMesh.material.opacity = 1.0 + pulse * 0.3;
      }
    });
